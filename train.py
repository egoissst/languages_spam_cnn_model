...
# STORING CONFIG PROPERTIES IN A FILE ##
config_file_str = get_file_string_from_config_props(FLAGS, cfg)
...
print('Train data : ratio -> {}'.format(sum(y_train)/len(y_train)))
print('Dev data : ratio -> {}'.format(sum(y_dev)/len(y_dev)))
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        config_details_dir = os.path.join(out_dir, "config_details.txt")
        with open(config_details_dir, 'w+') as f:
            print('Writing config details of the run to : {}'.format(config_details_dir))
